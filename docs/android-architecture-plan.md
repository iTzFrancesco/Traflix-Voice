...
### Mobile-only update channel

Android does not load the desktop Tauri updater. The native
`MobileUpdatePlugin` checks the public GitHub releases API asynchronously after
the Hub has rendered, filters out drafts and every tag that does not match
`android-vX.Y.Z`, and requires the exact `app-universal-release.apk` asset.
Windows tags and MSI assets therefore cannot be selected by the mobile path.

On startup, the Hub checks for a newer Android release and automatically starts
the update flow. The APK is downloaded into the application cache, checked
against the HTTPS GitHub release path, bounded asset size, published SHA-256,
and the device's native ABI, then opened through the existing `FileProvider`.
Android may first ask the user to allow installs from Traflix Voice and still
requires the final confirmation in the system installer. A release must keep
both values monotonic: the public tag is the mobile product version, while
`bundle.android.versionCode` must remain higher than every installed preview.

The current mobile build uses tag `android-v0.1.5`, versionName `0.1.5`, and
versionCode `1006003`, which is higher than the earlier preview's
`versionCode=1006002`. The custom updater is intentionally not shared with the
Windows shell, so desktop release behavior remains unchanged.
...
- The Hub displays the same session status and history whether or not its WebView
  is currently open.
- The Hub checks only mobile GitHub releases and lets the user start a signed
  APK update through the Android package installer.
