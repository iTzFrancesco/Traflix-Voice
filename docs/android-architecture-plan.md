...
The Hub shows an update card but does not interrupt startup or install without
consent. A user action downloads the APK into the application cache, validates
the HTTPS GitHub release path and bounded asset size, and opens Android's
package installer through the existing `FileProvider`. Android may first ask
the user to allow installs from Traflix Voice; the installer performs the
final package signature and version checks. A release must keep both values
monotonic: the public tag is the mobile product version, while
`bundle.android.versionCode` must remain higher than every installed preview.

The current mobile build uses tag `android-v0.1.4`, versionName `0.1.4`, and
versionCode `1006002`, which is higher than the earlier preview's
`versionCode=1006001`. The custom updater is intentionally not shared with the
